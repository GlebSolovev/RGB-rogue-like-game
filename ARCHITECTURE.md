...
* Методы `onStart()` и `onStop()` предоставляют возможность конкретной реализации `Behaviour`-а сделать дополнительные
  действия в момент собственного запуска или остановки. См. подробнее далее, такая возможность может быть необходимой
  для сложных `MetaBehaviour`-ов, которые внутри себя скрыто поддерживают другие `Behaviour`-ы.
