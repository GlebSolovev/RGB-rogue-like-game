...
// single-unit (bc entities are indivisible)
class Glitch(cell: Cell, hp: Int) : GameEntity(setOf(ColorHpCell(GameColor(RGB(0, 0, 0)), hp, cell))) {
    ...
    override suspend fun handleGameMessage(m: Message): Unit = when (m) {
        is RepaintTick -> Unit.also {
            val newUnits = units.map { GameUnit(this, it.cell, it.hp, GameColor(generateRandomColor(random))) }
            units.clear()
            units.addAll(newUnits)
            controller.view.receive(EntityMoved(this)) // TODO: updated, not moved
        }
        is MoveTick -> Unit.also {
            ...
            if (controller.physics.isEmpty(setOf(targetCell))) {
                ...
                if (controller.creation.tryAddToWorld(clone)) {
                    controller.view.receive(EntityMoved(clone))
                }
            }
        }
        // TODO
